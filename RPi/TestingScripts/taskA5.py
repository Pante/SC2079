...
class TaskA5:
    ...
    def stream_start(self):
        StreamServer().start(resolution=(640, 480), framerate=20, quality=45)
    ...
    def normalPathfinding(self):
        ...
        def add_obstacle(dir, sw):
            x, y = sw
            img_id = len(obstacleArr) + 1
            pathObstacle = PathfindingRequestObstacle(
                direction=Direction(dir),
                image_id=img_id,
                north_east=PathfindingPoint(x=x + 4, y=y + 4),
                south_west=PathfindingPoint(x=x, y=y),
            )
            obstacleArr.append(pathObstacle)

        # for dir_str in ["SOUTH", "EAST", "NORTH", "WEST"]:
        # 	add_obstacle(dir_str, (45, 40), (49, 44), 1)

        add_obstacle("WEST", (80, 50))
        add_obstacle("SOUTH", (35, 45))
        add_obstacle("WEST", (50, 70))
        add_obstacle("SOUTH", (65, 95))
        add_obstacle("NORTH", (70, 20))

        robot_direction = Direction("NORTH")
        robot_south_west = PathfindingPoint(x=0, y=0)
        robot_north_east = PathfindingPoint(x=11, y=11)
        ...
        for i, segment in enumerate(segments):
            ...
            for instruction in segment.instructions:
                ...
                self.stm.send_cmd(flag, 95, angle, val)
                # time.sleep(0.2)
